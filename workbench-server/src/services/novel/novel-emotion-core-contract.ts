...
export const EMOTION_CORE_HATE =
  '恨=冲突前置：压力方对白/动作立刻落地，当场亮谁要什么+可见代价（按题材自定）；**金额/期限/证物等条款优先在本拍一次说清**；规则压迫可细；困境嵌对白。禁空骂催醒、禁压力后感官环境泄压、禁盘点当恨场主线。'

export const EMOTION_CORE_SHUANG =
  '爽=动作震慑+本事露尖：可见动作改变当场权力关系 + 对在场物/证一句越界细节（本事本拍可视化首亮）；立约可留但不得单独当爽；**禁止重报已立金额/期限包**；接招须短句/动作勿整段复读条款；禁拖到盼才首亮。'

export const EMOTION_CORE_JI =
  '急=倒计时压迫感（停在开口）：用手段升级、起止锚、在场怕怂、资源不够喘来加压；**不是再宣读一遍合同**。恨拍已立N日则本拍禁止另报M日、禁止完整重喊「N日+要什么」；最多半句回指限期或钉起止锚服务已立时钟。恨拍未立期限时才可首次尖报天数。禁无说明叠「N日」与「明日」两套交割；禁只说软期限。'

export const EMOTION_CORE_PAN =
  '盼=短：复验已亮本事或一句缺一环；禁本拍首次亮本事、禁展开翻找/验契过程、禁温情泄压、禁再报一遍期限条款。'
